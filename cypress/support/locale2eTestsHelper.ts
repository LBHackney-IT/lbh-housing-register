...
export const visitHomepageSignInAndVerify = (
  ...
  cy.intercept('POST', '**/api/resident-auth/generate').as('localAuthGenerate'); // This intercept doens't actually mock anything. We capture the request so we can wait for sign-in backend completion before entering the verification code. This avoids breaking tests on first load.
