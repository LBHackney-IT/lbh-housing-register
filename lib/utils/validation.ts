    ...
    if (field.validation || field.type) {
      ...
      switch(fieldType) {
        case "checkbox":
          ...
          fieldValidation = checkRequired(fieldValidation, field, `${field.label} must be selected`)
          break;
        ...
        case "checkboxes":
          ...
          fieldValidation = baseType
          fieldValidation = checkRequired(fieldValidation, field, "At least one option must be selected")
          fieldValidation = checkMinimumLength(fieldValidation, field, `No less than ${field.validation?.min} item${field.validation?.min! > 1 ? "s" : ""} can be selected`)
          fieldValidation = checkMaximumLength(fieldValidation, field, `No more than ${field.validation?.max} item${field.validation?.max! > 1 ? "s" : ""} can be selected`)
          break;
        ...
        case "date":
          ...
          fieldValidation = checkMinimumLength(fieldValidation, field, `${field.label} must be on or after ${field.validation?.min}`)
          fieldValidation = checkMaximumLength(fieldValidation, field, `${field.label} must be on or before ${field.validation?.max}`)
          break;
        ...
        case "number":
          baseType = Yup.number().integer(`${field.label} must be a valid number`)

          fieldValidation = baseType
          fieldValidation = checkRequired(fieldValidation, field)
          fieldValidation = checkMinimumLength(fieldValidation, field, `${field.label} must be no less than ${field.validation?.min}`)
          fieldValidation = checkMaximumLength(fieldValidation, field, `${field.label} must be no more than ${field.validation?.max}`)
          break;
      ...
      fieldValidation = setUpConditionalValidation(fieldValidation, field, baseType)
 ...
 */
function checkMaximumLength(fieldValidationSchema: any, field: FormField, errorMessage?: string): Yup.BaseSchema {
 ...
 * @returns {Yup.BaseSchema} - The yup schema
 */
function checkMinimumLength(fieldValidationSchema: any, field: FormField, errorMessage?: string): Yup.BaseSchema {
 ...
 * @returns {Yup.BaseSchema} - The yup schema
 */
function checkRequired(fieldValidationSchema: Yup.BaseSchema, field: FormField, errorMessage?: string): Yup.BaseSchema {
  errorMessage = errorMessage || `${field.label} is required`
  return field.validation?.required ? fieldValidationSchema.required(errorMessage) : fieldValidationSchema
}
 ...
 */
function setUpConditionalValidation(fieldValidationSchema: Yup.BaseSchema, field: FormField, baseType: Yup.BaseSchema): Yup.BaseSchema {
  if (field.conditionalDisplay) {
    const conditionalLogicFields: string[] = field.conditionalDisplay.map(condition => condition.field)
